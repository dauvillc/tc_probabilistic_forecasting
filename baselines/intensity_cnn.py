...
if __name__ == "__main__":
    ...
    for epoch in range(n_epochs):
        ...
        model.train()
        for past_vars, past_data, y in tqdm(train_loader):
            # past_vars is the past trajectory variables (e.g. lat, lon...)
            # past_data is the past datacube.
            x = past_data
        ...
        model.eval()
        with torch.no_grad():
            for past_vars, past_data, y in val_loader:
                x = past_data
                x, y = x.to(device), y.to(device)
                y_pred = model(x)
                loss = loss_fn(y_pred, y)
