    ...
    if use_cache:
        ...
        for sid in iterator:
            ...
            if os.path.exists(os.path.join(path, str(year), sid + ".nc")):
                ...
                sid_rows = storms_dataset.merge(
                    sid_times, left_on=["SID", "ISO_TIME"], right_index=True
                )[["SID", "ISO_TIME"]]
...
def load_era5_patches(storms_dataset, load_atmo=True, load_surface=True):
    ...
    if load_atmo:
        print("Loading atmospheric patches...")
        atmo_patches = xr.open_mfdataset(
            os.path.join(path, "*_atmo_patches.nc"), combine="nested", concat_dim="sid_time"
        )
        # Select the patches corresponding to the storms in storms_dataset
        atmo_patches = select_sid_time(
            atmo_patches, storms_dataset["SID"], storms_dataset["ISO_TIME"]
        )
    ...
    if load_surface:
        ...
        surface_patches = xr.open_mfdataset(
            os.path.join(path, "*_surface_patches.nc"), combine="nested", concat_dim="sid_time"
        )
        surface_patches = select_sid_time(
            surface_patches, storms_dataset["SID"], storms_dataset["ISO_TIME"]
        )
