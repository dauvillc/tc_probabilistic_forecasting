...
def select_sid_time(datacube, sids, times):
    ...
    if "sid" not in datacube.coords or "time" not in datacube.coords:
        raise ValueError(
            "The 'sid_time' dimension must have the associated coordinates 'sid' and 'time'."
        )
...
def set_relative_spatial_coords(datacube, lat_dim="latitude", lon_dim="longitude"):
    ...
    datacube = datacube.assign_coords(
        v_pixel_offset=("v_pixel_offset", vertical_coords),
...
def rescale_and_crop(datacube, dims, new_res=None, crop_size=None):
    ...
    if new_res is not None:
        ...
        if crop_size is not None:
            ...
            new_lat = new_lat[
                center_idx - crop_size // 2 : center_idx + crop_size // 2 + crop_size % 2
            ]
            new_lon = new_lon[
                center_idx - crop_size // 2 : center_idx + crop_size // 2 + crop_size % 2
            ]
